...
export default function HeroSlider({ ringtones }: { ringtones: Ringtone[] }) {
  ...
  return (
    <div 
      className="relative w-full h-[420px] mb-6 flex items-center justify-center overflow-hidden"
                ...
                >
                   ...
                   <div className="relative w-full h-full rounded-2xl overflow-hidden bg-neutral-800 border border-white/10">
                      ...
                      <div className={`absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent transition-opacity duration-300 ${idx === currentIndex ? 'opacity-100' : 'opacity-60'}`}>
                         <div className={`absolute bottom-0 left-0 right-0 p-5 transition-all duration-300 ${idx === currentIndex ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                            <span className="inline-block px-2 py-0.5 bg-emerald-500 text-black text-[10px] font-bold rounded mb-2 shadow-lg shadow-emerald-500/20">TRENDING #{idx + 1}</span>
                            <h2 className="text-2xl font-bold text-white leading-none mb-1 drop-shadow-md">{ringtone.movie_name}</h2>
                            <p className="text-zinc-300 text-sm mb-4 line-clamp-1 drop-shadow-sm">{ringtone.title}</p>
                            
                            <div className="flex gap-2">
                              <Link 
                                href={`/ringtone/${ringtone.slug}`}
                                className="flex-1 bg-white text-black py-2.5 rounded-xl font-bold text-xs flex items-center justify-center gap-1 hover:bg-zinc-200 transition-colors shadow-lg"
                              ...
                              <Link 
                                ...
                                className="px-4 py-2.5 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 text-white text-xs font-bold hover:bg-white/20 transition-colors"
                                onClick={(e) => e.stopPropagation()}
                              >
                                Album
                              </Link>
